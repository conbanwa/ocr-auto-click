...
def match(target, entry):
    ...
    if clean_target == clean_entry:
        ...
        if target == entry:
            ...
            return levenshtein(target, entry) * containing(target, entry) * multiplier
    full_coefficient = levenshtein(target, entry) * containing(target, entry)
    return (levenshtein(clean_target, clean_entry) * containing(clean_target, clean_entry) *
            multiplier * (full_coefficient * 0.5 + 0.5))
...
def containing(target, entry, threshold=0.8):
    return contain(target, entry, threshold) * 0.8 + 0.2


def contain(short, long, threshold=0.8):
